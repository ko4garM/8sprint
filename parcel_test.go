...
func TestAddGetDelete(t *testing.T) {
	...
	new_id, err := store.Add(parcel)
	require.NoError(t, err)
	require.NotZero(t, new_id)
	parcel.Number = new_id

	// get
	// получите только что добавленную посылку, убедитесь в отсутствии ошибки
	// проверьте, что значения всех полей в полученном объекте совпадают со значениями полей в переменной parcel
	storedParcel, err := store.Get(new_id)
	require.NoError(t, err)
	...
	err = store.Delete(new_id)
	require.NoError(t, err)
	_, err = store.Get(new_id)
	require.Error(t, err)
}
...
func TestSetAddress(t *testing.T) {
	...
	err = store.SetAddress(new_id, newAddress)
	require.NoError(t, err)

	// check
	// получите добавленную посылку и убедитесь, что адрес обновился
	p, err := store.Get(new_id)
	require.NoError(t, err)
...
func TestSetStatus(t *testing.T) {
	...
	err = store.SetStatus(new_id, ParcelStatusSent)
	require.NoError(t, err)
	// check
