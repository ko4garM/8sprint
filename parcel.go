...
func (s ParcelStore) Get(number int) (Parcel, error) {
	...
	p := Parcel{}
	res := s.db.QueryRow("SELECT number,client,status,address,created_at FROM parcel WHERE number = :id",
		sql.Named("id", number))
...
func (s ParcelStore) Delete(number int) error {
	...
	_, err := s.db.Exec("DELETE FROM parcel WHERE number =:id AND status=:status", sql.Named("id", number), sql.Named("status", ParcelStatusRegistered))
	return err
}
